...
int main (){
	...
	/* Computes the error value - if positive, white line is on one side
	 * of image, if negative white line is on other side, if zero in middle */	
	...
	/* Robot moves back very slowly if it only sees black, meaning it has lost the white line */
	 
		while (max < 100)
		{
			set_motor(1, - (vgo_m1/10));
			set_motor(2, - (vgo_m2/10));
		}
